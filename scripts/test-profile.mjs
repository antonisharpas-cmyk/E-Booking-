...
let pid=[...pricing.text.matchAll(/"id":"([0-9a-f-]{36})","slug":"pack-10"/g)][0]?.[1];
if(!pid) pid=pricing.text.match(/\\"id\\":\\"([0-9a-f-]{36})\\",\\"slug\\":\\"pack-10\\"/)?.[1];
check('found the 10-class pack id', Boolean(pid), pid);
const grant=await req('/api/checkout',{method:'POST',body:{packageId:pid}});
check('dev grant gives 10 sessions', grant.json?.credits===10, grant.json);
...
jar.clear();
await req('/api/auth/login',{method:'POST',body:{email:'admin@apexpilates.cy',password:'apexadmin123'}});
r=await req('/api/reminders/due');
